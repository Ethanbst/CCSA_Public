...
from conseil_communautaire.models import ConseilVille, ConseilMembre
import shutil
import os
...
class ConseilVilleViewsTestCase(TestCase):
    ...
    def test_general_view_without_data(self):
        ...
        self.assertContains(response,
                            "Aucune commune ni membre n'est disponible pour le moment.")
    ...
    def test_add_conseil_ville_view(self):
        ...
        response = self.client.post(reverse('conseil_communautaire:admin_add_city'),
                                    data=self.data, follow=True)
        # On suit la redirection (si elle avait eu lieu)
        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, reverse('conseil_communautaire:admin_list_cities'))
        # Vérifie que le nom de la ville est présent dans la réponse
        self.assertContains(response, "Test City")
        # Vérifie qu'on arrive bien sur la liste
        self.assertTemplateUsed(response,
                                'conseil_communautaire/admin_cities_list.html')
    ...
    def test_edit_conseil_ville_view(self):
        ...
        """
        response = self.client.get(reverse('conseil_communautaire:admin_edit_city',
                                           args=[self.city.id]))
        ...
        response = self.client.post(reverse('conseil_communautaire:admin_edit_city',
                                            args=[self.city.id]), data=updated_data,
                                    follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, reverse('conseil_communautaire:admin_list_cities'))
    ...
    def test_edit_conseil_ville_view_invalid_data(self):
        ...
        response = self.client.post(reverse('conseil_communautaire:admin_edit_city',
                                            args=[self.city.id]), data=invalid_data)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'conseil_communautaire/admin_city_edit.html')
    ...
    def test_delete_conseil_ville_view(self):
        ...
        """
        response = self.client.get(reverse('conseil_communautaire:admin_delete_city',
                                           args=[self.city.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
                                'conseil_communautaire/admin_city_delete.html')
        self.assertContains(response, "Êtes-vous certain de vouloir supprimer : ")
        self.assertContains(response, self.city.city_name)

        # Envoie une requête POST pour supprimer la ville
        response = self.client.post(reverse('conseil_communautaire:admin_delete_city',
                                            args=[self.city.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, reverse('conseil_communautaire:admin_list_cities'))
        self.assertFalse(ConseilVille.objects.filter(city_name="First City").exists())
...
class ConseilMembreViewsTestCase(TestCase):
    ...
    def test_view_list_conseil_membre(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
                                'conseil_communautaire/admin_members_list.html')
    ...
    def test_add_conseil_membre_view_valid_data(self):
        ...
        response = self.client.post(reverse('conseil_communautaire:admin_member_add'),
                                    data=self.data, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_conseil_membre_view(self):
        ...
        """
        self.init_test_data()
        response = self.client.get(reverse('conseil_communautaire:admin_member_edit',
                                           args=[self.member.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
        ...
        response = self.client.post(reverse('conseil_communautaire:admin_member_edit',
                                            args=[self.member.id]),
                                    data=updated_data,
                                    follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_conseil_membre_view_invalid_data(self):
        ...
        response = self.client.post(reverse('conseil_communautaire:admin_member_edit',
                                            args=[self.member.id]), data=invalid_data)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
                                'conseil_communautaire/admin_member_edit.html')
        self.assertContains(response, "Ce champ est obligatoire.")
    ...
    def test_delete_conseil_membre_view(self):
        ...
        """
        self.init_test_data()
        response = self.client.get(reverse('conseil_communautaire:admin_member_delete',
                                           args=[self.member.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
                                'conseil_communautaire/admin_member_delete.html')
        self.assertContains(response, "Êtes-vous certain de vouloir supprimer : ")
        self.assertContains(response, "Jane")
        self.assertContains(response, "SMITH")

        # Envoie une requête POST pour supprimer le membre
        response = self.client.post(reverse('conseil_communautaire:admin_member_delete',
                                            args=[self.member.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, reverse('conseil_communautaire:admin_list_cities'))
        self.assertTemplateUsed(response,
                                'conseil_communautaire/admin_cities_list.html')
