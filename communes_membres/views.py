...
def commune(request, slug):
    ...
    if ActeLocal.objects.filter(commune=commune).exists():
        ...
        acte = ActeLocal(id="0", title=None, date=None, description=None,
                         commune=commune, file="/")
...
def list_acte_local(request):
    ...
    return render(request, 'communes_membres/admin_acte_list.html',
                  {'actes_locaux': actes_locaux})
...
def delete_acte_local(request, id):
    ...
    return render(request, 'communes_membres/admin_acte_delete.html',
                  {'acte_local': acte_local})
