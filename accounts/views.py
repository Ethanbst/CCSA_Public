...
def register_view(request):
    ...
    if user_count > 0:
        messages.error(
            request,
            _("L'inscription n'est plus disponible. Veuillez contacter un "
              "administrateur.")
        )
    ...
    if request.method == 'POST':
        ...
        if user_id and action:
            ...
            if user.is_superuser and user != request.user:
                messages.error(
                    request,
                    _("Vous ne pouvez pas modifier un autre "
                      "superadministrateur.")
                )
            ...
            if action == 'activate':
                ...
                messages.success(
                    request,
                    _(f"L'utilisateur {user.username} a été activé.")
                )
                ...
                messages.success(
                    request,
                    _(f"L'utilisateur {user.username} a été désactivé.")
                )
                ...
                messages.success(
                    request,
                    _(f"L'utilisateur {username} a été supprimé.")
                )
...
def admin_create_user(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                _(f"L'utilisateur {user.username} a été créé avec succès.")
            )
            return redirect('accounts:admin_user_list')
    else:
        form = CustomUserCreationForm()
