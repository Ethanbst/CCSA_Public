...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(
        _('nom d\'utilisateur'), max_length=150, blank=True
    )
    is_staff = models.BooleanField(
        _('statut staff'),
        default=False,
        help_text=_(
            'Désigne si l\'utilisateur peut se connecter au site '
            'd\'administration.'
        ),
    )
    ...
    date_joined = models.DateTimeField(
        _('date d\'inscription'), default=timezone.now
    )
    last_login = models.DateTimeField(
        _('dernière connexion'), null=True, blank=True
    )
