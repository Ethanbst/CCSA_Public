...
class AdminCreateUserTests(TestCase):
    def setUp(self):
        ...
        self.client.login(username=self.superuser_data['email'], password=self.superuser_data['password1'])
    ...
    def test_admin_create_user_page_post_views(self):
        ...
        """
        response = self.client.post(reverse('accounts:admin_create_user'), self.user_data)
        self.assertEqual(response.status_code, 302)
